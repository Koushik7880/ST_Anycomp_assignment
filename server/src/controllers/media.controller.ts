...
export const listMediaBySpecialist = async (req: Request, res: Response) => {
  try {
...
export const createMediaForSpecialist = async (req: Request, res: Response) => {
  try {
    const { specialistId } = req.params;
    ...
    const created = await Promise.all(
      files.map((file, index) => {
        const display_order =
          displayOrders[index] ?? // if provided
          index + 1;              // fallback
