...
exports.exportSpecialists = exports.softDeleteSpecialist = exports.publishSpecialist = exports.updateSpecialist = exports.getSpecialist = exports.createSpecialist = exports.listSpecialists = void 0;
...
const listSpecialists = async (req, res) => {
    try {
        ...
        const [specialists, totalSpecialists] = await Promise.all([
            prisma_1.prisma.specialists.findMany({
                where,
                skip,
                take: limitNum,
            ...
            prisma_1.prisma.specialists.count({ where }),
        ]);
        res.json({
            success: true,
            data: specialists,
...
exports.listSpecialists = listSpecialists;
// POST /api/specialists
const createSpecialist = async (req, res) => {
    try {
        const { title, slug, description, base_price, platform_fee, final_price, duration_days, average_rating = 0, is_draft = true, total_number_of_ratings = 0, verification_status = "pending", is_verified = false, additional_offerings, company_secretary, } = req.body;
        const now = new Date();
        const data = {
            title,
            slug,
        ...
        const newSpecialist = await prisma_1.prisma.specialists.create({ data });
...
const getSpecialist = async (req, res) => {
    try {
        const { id } = req.params;
        const specialist = await prisma_1.prisma.specialists.findFirst({
...
const updateSpecialist = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, slug, description, base_price, platform_fee, final_price, duration_days, average_rating, is_draft, total_number_of_ratings, verification_status, is_verified, additional_offerings, company_secretary, } = req.body;
        const data = {
            updated_at: new Date(),
        };
        if (title !== undefined)
        ...
        if (base_price !== undefined)
            data.base_price = base_price.toString();
        if (platform_fee !== undefined)
            data.platform_fee = platform_fee.toString();
        if (final_price !== undefined)
            data.final_price = final_price.toString();
        ...
        if (company_secretary !== undefined)
            data.company_secretary = company_secretary;
        const updatedSpecialist = await prisma_1.prisma.specialists.update({
            where: { id },
            data,
        });
...
const publishSpecialist = async (req, res) => {
    try {
        const { id } = req.params;
        const publishedSpecialist = await prisma_1.prisma.specialists.update({
            where: { id },
            data: {
                is_draft: false,
                updated_at: new Date(),
...
exports.publishSpecialist = publishSpecialist;
// DELETE /api/specialists/:id
const softDeleteSpecialist = async (req, res) => {
    try {
        const { id } = req.params;
        await prisma_1.prisma.specialists.update({
            where: { id },
            data: {
                deleted_at: new Date(),
                updated_at: new Date(),
...
exports.softDeleteSpecialist = softDeleteSpecialist;
const exportSpecialists = async (req, res) => {
    try {
        ...
        const specialists = await prisma_1.prisma.specialists.findMany({
            where,
            orderBy: { created_at: "desc" },
        });
        // --- Build CSV ---
        ...
        res.setHeader("Content-Disposition", "attachment; filename=specialists-export.csv");
...
exports.exportSpecialists = exportSpecialists;
